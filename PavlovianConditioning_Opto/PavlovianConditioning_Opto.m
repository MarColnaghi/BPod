...
CS0Trials = [60 1];                       % Valve Click - [Number of Trials, Code]
CS1Trials_R = [57 2];                     % CS+ - [Number of Rewarded Trials, Code]
CS1Trials_nR = [3 3];                     % CS+ - [Number of non Rewarded Trials, Code]
...
for currentTrial = 1: S.GUI.MaxTrials
    ...
    switch trialTypes(currentTrial)
        ...
        % Valve Click
        case 1
            StimulusArgument= {'ValveModule1', 3};        % Send TTL to DAQ (Stimulus Delivery)
            FollowingLED = 'NothingHappens';
        
        % CS+ Reward
        case 2
            StimulusArgument= {'ValveModule1', 4};        % Send TTL to DAQ (Stimulus Delivery)
            FollowingLED = 'Reward';                               

        % CS+ no Reward
        case 3
            StimulusArgument= {'ValveModule1', 4};        % Send TTL to DAQ (Stimulus Delivery)
            FollowingLED = 'NothingHappens';                        % No reward given
...
%%%%%%%
    ...
    sma= AddState(sma, 'Name', 'PreStimulus',...
        ...
        'OutputActions', {});                                       % Starts Camera Acquisition                         
    ...
    sma = AddState(sma, 'Name', 'LEDon', ...
        ...
        'StateChangeConditions', {'Tup', FollowingLED},...
...
cb = brewermap(10,'Set3')
state_colors = struct( ...
    'PreStimulus', cb(1,:),...
    'DeliverStimulus', cb(2,:),...
    'StopStimulus',  cb(3,:),...
    'Pause', cb(4,:),... 
    'LEDon', cb(5,:),...
    'Reward', cb(6,:),...
    ...
    'NothingHappens', cb(8,:),...
    'EndTrial', cb(9,:),...
    'InterTrialInterval', cb(10,:));

function poke_colors = getPokeColors
poke_colors = struct( ...
      'Port1In', 0.6*[1 0.66 0])
